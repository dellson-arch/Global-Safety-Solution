...
export class QuotationsService {
  ...
  async create(data: any) {
    ...
    const quoteNumber = `GSS/${year}/${String(count + 1).padStart(3, '0')}`;
  ...
  async convertToProjectAndInvoice(id: string) {
    return this.prisma.$transaction(
      ...
      if (existingInvoice) {
        throw new Error('This quotation has already been converted to a Project and Invoice.');
      }
      ...
      if (!clientId) {
        throw new Error('Cannot convert quotation: No associated client found. Ensure lead is converted to client first.');
      }
      ...
      for (const item of quotation.items) {
        ...
        const workOrderNo = `WO-${new Date().getFullYear()}-${(woCount + 1).toString().padStart(4, '0')}`;
