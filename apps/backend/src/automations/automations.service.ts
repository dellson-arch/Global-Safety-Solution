...
export class AutomationsService {
  ...
  async handleOverdueInvoices() {
    ...
    for (const invoice of overdueInvoices) {
      this.logger.warn(`Alert: Invoice ${invoice.invoice_number} for client ${invoice.client.name} is OVERDUE.`);
  ...
  async handleLeadFollowUps() {
    ...
    for (const lead of leadsToFollowUp) {
       this.logger.log(`Reminder: Follow up with lead ${lead.company_name} today.`);
        ...
        for (const admin of adminRoles) {
          await this.prisma.notification.create({
            data: {
              ...
              title: days === 0 ? `EXPIRED: ${cert.certificate_no}` : `Renewal Alert: ${cert.certificate_no}`,
              message: message,
